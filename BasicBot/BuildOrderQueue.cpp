...
const std::deque< BuildOrderItem > BuildOrderQueue::viewNextItems()
{
	assert(queue.size() - 1 - numSkippedItems >= 0);

	// the queue will be sorted with the highest priority at the back
	std::deque< BuildOrderItem > viewQ;
	for (int i = queue.size() - 1 - numSkippedItems; i < int(queue.size()); i++){
		viewQ.push_back(queue[i]);
	}
...
void BuildOrderQueue::skipCurrentItem()
{
	...
	if (canSkipCurrentItem()) {
		// skip it
		numSkippedItems++;
		skippedGas += queue[queue.size() - 1 - numSkippedItems].metaType.gasPrice();
		skippedMinerals += queue[queue.size() - 1 - numSkippedItems].metaType.mineralPrice();
	}
}
...
std::pair<int, int> BuildOrderQueue::getSkippedResource(){
	std::pair<int, int> rst(0, 0);

	rst.first = skippedMinerals;
	rst.second = skippedGas;
