	...
	// seedPosition 을 입력하지 않은 경우
		...
		case BuildOrderItem::SeedPositionStrategy::MissileTurret:
			if (InformationManager::Instance().getTurretStatus() == false) {
				...
				desiredPosition = getBuildLocationNear(buildingType, InformationManager::Instance().getTurretPosition());
				break;
			}
		...
		case BuildOrderItem::SeedPositionStrategy::SecondExpansionLocation:
			if (InformationManager::Instance().getOccupiedBaseLocations(BWAPI::Broodwar->self()).size() > 1) {
				...
				if (tempTilePosition != BWAPI::TilePositions::None) {
					...
					desiredPosition = getBuildLocationNear(buildingType, BWAPI::TilePosition(tempTilePosition));
				}
		...
		case BuildOrderItem::SeedPositionStrategy::SupForWall:
			{
				...
				desiredPosition = InformationManager::Instance().getSupPostionsForWall().front();
				InformationManager::Instance().setWallPosition(desiredPosition);
				InformationManager::Instance().getSupPostionsForWall().pop();
			}
			break;
...
BWAPI::TilePosition	ConstructionPlaceFinder::getBuildLocationNear(BWAPI::UnitType buildingType, BWAPI::TilePosition desiredPosition, int buildingGapSpace, size_t constructionPlaceSearchMethod) const
{
	...
	if (constructionPlaceSearchMethod == ConstructionPlaceSearchMethod::SpiralMethod)
	{
		...
		while (spiralMaxLength < maxRange)
		{
			...
			{
				bool passFlag = false;
				//같은 리젼에만 짓도록 - 로템, 투혼에서 서플라이만 
				if (buildingType == BWAPI::UnitTypes::Terran_Supply_Depot && 
					//InformationManager::Instance().getMapName() != 'H' &&
					!seedRegion->getPolygon().isInside(BWAPI::Position(currentX * 32, currentY * 32))){
					passFlag = true;
				}
...
bool ConstructionPlaceFinder::canBuildHereWithSpace(BWAPI::TilePosition position, const ConstructionTask & b, int buildingGapSpace) const
{
	...
	else
	{
		...
		if (!isLeftTopOnly(b.type))
		{
			...
			if (UnitUtils::GetAllUnitCount(BWAPI::UnitTypes::Terran_Supply_Depot) % 2 == 0){
		...
		for (int y = position.y; y < position.y + height; y++)
		{
			...
			if (isReservedTile(position.x - 3, y))
			{
				return false;
			}
			for (auto & unit : BWAPI::Broodwar->getUnitsOnTile(position.x - 3, y))
	...
	*/
	...
	for (auto & geyser : BWAPI::Broodwar->getStaticGeysers())
	{
		...
		if (refineryAlreadyBuilt == false)
		{
			...
			double thisDistance = MapTools::Instance().getGroundDistance(geyserPos, BWAPI::Position(seedPosition));
