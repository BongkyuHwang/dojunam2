...
WorkerData::WorkerData() 
{
	...
	mineralAndMineralWorkerRatio = 1.5;  // 1.5로 수정
...
bool WorkerData::depotHasEnoughMineralWorkers(BWAPI::Unit depot)
{
	...
	if (assignedWorkers > (int)(mineralsNearDepot * 2 * weight))
	{
		return true;
	}
	else
...
int WorkerData::getRepairUnitCountOneTarget(BWAPI::Unit unit) {
	int count = 0;
	std::map<BWAPI::Unit, BWAPI::Unit>::iterator it;
	for (it = workerRepairMap.begin(); it != workerRepairMap.end(); ++it) {
		if (it->second == unit)
			count++;
	}
	return count;
