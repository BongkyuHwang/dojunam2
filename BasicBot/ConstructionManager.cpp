...
// SCV 앞길을 막는 유닛들은 비켜라!
void moveAway(BWAPI::Unit unit, BWAPI::TilePosition targetTilePosition)
{
		...
		if (b.status == ConstructionStatus::UnderConstruction)
		{
			...
			//커맨드 센터는 취소
				...
				if (b.buildingUnit->getType() == BWAPI::UnitTypes::Terran_Command_Center){
					...
					//if (BWAPI::Broodwar->getFrameCount() % 24 == 0) //std::cout << "Command center is under attack!!!(LTD:" << value_LTD << ")" << std::endl;
				}
			}
		}
    }
...
void ConstructionManager::assignWorkersToUnassignedBuildings()
{
    ...
    for (ConstructionTask & b : constructionQueue)
    {
		...
		if (testLocation == BWAPI::TilePositions::None || testLocation == BWAPI::TilePositions::Invalid || testLocation.isValid() == false) {
			...
			//std::cout << "No Worker : " << testLocation << std::endl;
		...
		//std::cout << "No Worker : chooseConstuctionWorkerClosestTo" << testLocation << std::endl;
    }
}
		...
		*/
		...
		else if (b.constructionWorker->isConstructing() == false)        
        {
			...
			else
            {
				...
				if (BWAPI::Broodwar->getFrameCount() - b.lastBuildCommandGivenFrame > 12) {
