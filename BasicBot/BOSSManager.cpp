...
{
	size_t numWorkers = UnitUtils::GetAllUnitCount(BWAPI::Broodwar->self()->getRace().getWorker());
	size_t numDepots = UnitUtils::GetAllUnitCount(BWAPI::Broodwar->self()->getRace().getCenter())
						+ UnitUtils::GetAllUnitCount(BWAPI::UnitTypes::Zerg_Lair)
						+ UnitUtils::GetAllUnitCount(BWAPI::UnitTypes::Zerg_Hive);
    ...
    // convert from MyBot's meta goal type to BOSS ActionType goal
    try
    {
        BOSS::BuildOrderSearchGoal goal = GetGoal(goalUnits);

		BOSS::GameState initialState(BWAPI::Broodwar, BWAPI::Broodwar->self(), ConstructionManager::Instance().buildingsQueued());
...
void BOSSManager::update(double timeLimit)
{
    ...
    if (isSearchInProgress())
    {
		...
		bool searchTimeOut = (BWAPI::Broodwar->getFrameCount() > (_previousSearchStartFrame + 120));
